...
try {
    ...
    foreach ($rows as $row) {
        ...
        $vehicles[] = [
            ...
            'type' => $row['category'] ?? '',
            'plate' => $row['plate_no'] ?? '',
            'seats' => array_key_exists('seats', $row) ? (int) $row['seats'] : null,
            'fuel' => array_key_exists('fuel_type', $row) ? ($row['fuel_type'] ?? '') : null,
            'trans' => array_key_exists('transmission', $row) ? ($row['transmission'] ?? '') : null,
            'rate' => (float) $row['daily_rate'],
            'mileage' => array_key_exists('mileage', $row) ? (int) $row['mileage'] : null,
            'status' => $status,
